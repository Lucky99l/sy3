...
def plot_trajs(length, height, pos1, pos2, pos3, episode, path):
    ...
    ax2.plot(x2, y2, 'b-', label='expect_traj')
    # print(x3)
    # print(len(y3))
    # if len(x3) != 0:
...
def plot_traj(step, length, height, pos1, pos2, episode, path):
    if not os.path.exists(path):
        os.mkdir(path)
    x1, y1, x2, y2 = [], [], [], []
    for i in range(len(pos1)):
    ...
    ax2.legend()
    plt.title('traj')
    fig2.savefig(path + '/' + str(episode) +'_' + str(step) + '.png', dpi=600, format='png')
    plt.close()
