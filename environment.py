...
num_gps = 100
num_slots = 32
num_pre_booked = random.randint(4*num_gps, 8*num_gps)
agent_pos = [0, 0]

num_to_book = 50
# num_to_book = random.randint(4*num_gps, 8*num_gps)
...
class SchedulerEnv(gym.Env):
    ...
    def cal_reward(self):
        up = max(0, self.old_agent_pos[0]-1)
    ...
    def get_score(self):
        score = 0
        up = max(0, self.old_agent_pos[0]-1)
        down = min(self.num_slots-1, self.agent_pos[0]+1)
        if (self.state[0, up, self.old_agent_pos[1]] == 0):
            if (self.state[0, down, self.agent_pos[1]] == 0):
                score = 0
            else:
                score = self.to_book[self.appt_idx - 1]
        else:
            if (self.state[0, down, self.agent_pos[1]] == 0):
                score = self.to_book[self.appt_idx - 1]
            else:
                score = 2 * self.to_book[self.appt_idx - 1]
    ...
    def step(self, action, old_action):
        ...
        if new_agent_pos == self.agent_pos or (old_action+action)==5 or (old_action+action)==1:
